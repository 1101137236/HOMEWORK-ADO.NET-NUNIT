    ...
    public partial class WebForm4 : System.Web.UI.Page
    {
        ...
        protected void btnSearch_Click(object sender, EventArgs e)
        {
            using (SqlConnection cn = new SqlConnection(ConfigurationManager.ConnectionStrings["PetConnectionString"].ConnectionString))
            {
                ...
                {
                    cmd.CommandText = "select * from pet where Name like @name";
        ...
        {
            string sql = @"INSERT INTO [dbo].[pet]([ID],[Name],[Gender],[Tel])
                             VALUES
                               (@ID
                               ,@Name
                               ,@Gender
                               ,@Tel);SELECT CAST(scope_identity() AS int);";
            using (SqlConnection cn = new SqlConnection(ConfigurationManager.ConnectionStrings["PetConnectionString"].ConnectionString))
            {
                cn.Open();
                using (SqlCommand cmd = cn.CreateCommand())
        ...
        {
            string sql = @"update [pet] set [Name] = @Name, [Gender] = @Gender, [Tel] = @Tel
                             where ID = @ID";
            using (SqlConnection cn = new SqlConnection(ConfigurationManager.ConnectionStrings["PetConnectionString"].ConnectionString))
            {
                cn.Open();
                using (SqlCommand cmd = cn.CreateCommand())
                {
                    ...
                    cmd.Parameters.Add(new SqlParameter("@Tel", txtE_Tel.Text));
